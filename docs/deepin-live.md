...
当然您也可以用其他 Linux 的 Live 系统，比如常见的 Ubuntu 及其衍生版、Manjaro、Veket 等，很多都能选择中文进入，并内置了输入法、Office 等软件。需要提醒的是，使用时不要产生太多数据以致系统盘占满；使用 Veket 前请阅读其说明；部分 Ubuntu 衍生版预装的 LibreOffice 无中文语言包，请执行 `sudo apt install libreoffice-l10n-zh-cn` 来安装它；Ubuntu 默认密码为无，Manjaro 默认密码为manjaro。
...
运行完毕后重启试试看。如果还是不行，抱歉，只能帮您到这了。操作过程中有错误信息的话可以搜索一下，寻找解决方法。
...
由于笔者水平有限，本文可能存在一些疏漏之处。如果您认为本文存在需要改进之处，欢迎指出。
